    ...
    return Stack(
      children: <Widget>[
        ...
        _DatePickerModeToggleButton(
          mode: _mode,
          title: _localizations.formatMonthYear(widget.isLunarDay
              ? Lunar(createdFromSolar: true, date: _currentDisplayedMonthDate)
                  .toDateTime()
              : _currentDisplayedMonthDate),
                    ...
                    child: Padding(
                      ...
                      child: Row(
                        children: <Widget>[
                          Flexible(
                            child: Text(
                              GetUtils.capitalizeFirst(widget.title) ??
                                  widget.title,
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void _setDateOver(int date) {
    setState(() {
      _dateOver = date;
      log('Date over: $_dateOver');
    });
  }
...
class _DayPicker extends StatefulWidget {
  ...
  _DayPicker({
    ...
    this.selectableDayPredicate,
    required this.isLunarDay,
    required this.setDateOver,
    required this.dateOver,
  })  : assert(!firstDate.isAfter(lastDate)),
        assert(selectedDate == null || !selectedDate.isBefore(firstDate)),
        assert(selectedDate == null || !selectedDate.isAfter(lastDate));
  ...
  final ValueChanged<int> setDateOver;
...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    while (day < 35) {
      day++;
      if (day < 1) {
        final int tmp = DateUtils.getDaysInMonth(
            (month - 1) > 0 ? year : year - 1,
            (month - 1) > 0 ? month - 1 : 12);
        final DateTime dayToBuild = DateTime((month - 1) > 0 ? year : year - 1,
            (month - 1) > 0 ? month - 1 : 12, tmp + day);
        ...
        dayItems.add(
          _Day(
            ...
            focusNode: _dayFocusNodes[(tmp + day) - 1],
            isLunarDay: widget.isLunarDay,
          ),
        );
...
class _DayState extends State<_Day> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color dayForegroundColor =
        widget.isToday ? ResColor.blue : const Color(0xFF222B45);
    ...
    Widget dayWidget = DecoratedBox(
      ...
      child: Center(
        child: Text(
            localizations.formatDecimal(widget.isLunarDay
                ? Lunar(createdFromSolar: true, date: widget.day).day
                : widget.day.day),
